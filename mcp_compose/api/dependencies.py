...
_composer: MCPServerComposer | None = None
_config: MCPComposerConfig | None = None
_role_manager: RoleManager | None = None
_authz_middleware: AuthorizationMiddleware | None = None
_tool_permission_manager: ToolPermissionManager | None = None
...
async def get_composer() -> MCPServerComposer:
    ...
    if _composer is None:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Composer not initialized"
        )
    return _composer
...
async def get_config() -> MCPComposerConfig:
    ...
    if _config is None:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Config not initialized"
        )
    return _config


async def get_role_manager() -> RoleManager | None:
    """Get the RoleManager instance."""
    return _role_manager


async def get_authz_middleware() -> AuthorizationMiddleware | None:
    """Get the AuthorizationMiddleware instance."""
    return _authz_middleware


async def get_tool_permission_manager() -> ToolPermissionManager | None:
...
async def require_permission(
    ...
    authz: AuthorizationMiddleware | None = Depends(get_authz_middleware),
) -> AuthContext:
    """
...
async def require_tool_permission(
    ...
    action: str = "execute",
    auth_context: AuthContext = Depends(require_auth),
    tool_mgr: ToolPermissionManager | None = Depends(get_tool_permission_manager),
) -> AuthContext:
    """
