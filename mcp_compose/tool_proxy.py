...
class ToolProxy:
    ...
    async def discover_tools(self, server_name: str, process: Process) -> None:
        ...
        try:
            ...
            if tools_response and "result" in tools_response:
                ...
                logger.info(
                    f"Discovered {len(tools)} tools from {server_name}: {[t.get('name') for t in tools]}"
                )
    ...
    def _register_tool_proxy(
        self, server_name: str, tool_name: str, tool_def: dict[str, Any], process: Process
    ) -> None:
        """
            ...
            for param_name, param_spec in properties.items():
                ...
                if param_name in required_params:
                    param = inspect.Parameter(
                        param_name, inspect.Parameter.POSITIONAL_OR_KEYWORD, annotation=python_type
                    )
                else:
                    param = inspect.Parameter(
                        param_name,
        ...
        try:
            ...
            tool_obj = Tool.from_function(
                proxy_func, name=prefixed_name, description=tool_def.get("description", "")
            )
            ...
            if input_schema:
                ...
                logger.debug(
                    f"Tool {prefixed_name} parameters after override: {json.dumps(input_schema, indent=2)}"
                )
            ...
            logger.info(
                f"Registered proxy tool: {tool_obj.name} with schema: {list(input_schema.get('properties', {}).keys())}"
            )
        except Exception as e:
...
def fix_tool_argument_model(tool_obj: Tool, input_schema: dict[str, Any]) -> None:
    ...
    try:
            ...
            if "anyOf" in field_schema:
                ...
                if main_type:
                    ...
                    if field_type == "array":
                        ...
                        python_type = Optional[dict[str, AnyType]]
        ...
        new_model = create_model(f"{tool_obj.name}_Args", __base__=ArgModelBase, **fields)
