...
class AuthMiddleware:
    ...
    async def validate_session(self, session_id: str) -> AuthContext | None:
        """
    ...
    def get_session_context(self, session_id: str) -> AuthContext | None:
        """
    ...
    def wrap_handler(
        self,
        handler: Callable,
        extract_credentials: Callable[[dict[str, Any]], dict[str, Any]] | None = None,
    ) -> Callable:
        """
    ...
    def require_scope(self, *scopes: str) -> Callable:
        ...
        def decorator(handler: Callable) -> Callable:
            async def wrapped_handler(request: dict[str, Any], **kwargs) -> Any:
                ...
                if missing:
                    raise InsufficientScopesError(f"Missing required scopes: {', '.join(missing)}")
    ...
    def clear_expired_sessions(self) -> int:
        ...
        """
        expired = [
            session_id for session_id, context in self._contexts.items() if context.is_expired()
        ]
...
def create_auth_middleware(authenticator: Authenticator, **kwargs) -> AuthMiddleware:
    """
