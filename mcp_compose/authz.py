...
class Permission:
    ...
    def from_string(cls, permission_str: str) -> "Permission":
        ...
        if len(parts) != 2:
            raise ValueError(
                f"Invalid permission format: {permission_str}. Expected format: resource:action"
            )
    ...
    def require_permission(
        ...
        def decorator(handler: Callable) -> Callable:
            async def wrapped_handler(request: dict[str, Any], **kwargs) -> Any:
                ...
                if not auth_context:
                    raise InsufficientScopesError("No authentication context available")

                if not self.check_permission(auth_context, resource, action):
                    raise InsufficientScopesError(f"Missing permission: {resource}:{action}")
...
def create_authorization_middleware(
    role_manager: RoleManager | None = None, **kwargs
