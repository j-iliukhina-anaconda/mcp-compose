...
def setup_otel_tracing(service_name: str = "mcp-compose", out=sys.stderr) -> Any | None:
    """
...
def _load_oauth_overrides_from_config_json(config_path: str | None) -> dict:
    ...
    try:
        ...
        if args.config:
            ...
            if config_path is None:
                ...
                print(
                    "No configuration file found. Starting without proxied MCP servers.",
                    file=sys.stderr,
                )
...
async def run_server(config, args: argparse.Namespace) -> int:
    ...
    from .auth import AuthType, create_authenticator
    from .composer import ConflictResolution, MCPServerComposer
    from .config import AuthProvider, StdioProxiedServerConfig
    ...
    if transport_mode is None:
        ...
        if (
            config.transport.stdio_enabled
            and not config.transport.streamable_http_enabled
            and not config.transport.sse_enabled
        ):
    ...
    if config.authentication.enabled:
        ...
        if provider == AuthProvider.ANACONDA:
            if config.authentication.anaconda:
                ...
                authenticator = create_authenticator(AuthType.ANACONDA, domain=domain)
            else:
    ...
    try:
        ...
        has_embedded = (
            hasattr(config, "servers")
            and hasattr(config.servers, "embedded")
            and hasattr(config.servers.embedded, "servers")
            and config.servers.embedded.servers
        )
        has_stdio = (
            hasattr(config, "servers")
            and hasattr(config.servers, "proxied")
            and hasattr(config.servers.proxied, "stdio")
            and config.servers.proxied.stdio
        )
        ...
        if has_embedded:
            print(
                f"📦 Composing {len(config.servers.embedded.servers)} embedded servers...", file=out
            )
            await composer.compose_from_config(config)
            print("   ✓ Embedded servers composed", file=out)
            print(file=out)
        ...
        if (
            hasattr(config, "servers")
            and hasattr(config.servers, "proxied")
            and hasattr(config.servers.proxied, "sse")
        ):
            import subprocess
            import time
                    ...
                    if isinstance(server_config, SseProxiedServerConfig):
                        ...
                        if server_config.auto_start and server_config.command:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Failed to auto-start SSE server {server_config.name}: {e}"
                                )
                        ...
                        try:
                            ...
                            async with sse_client(server_config.url) as (read, write):
                                async with ClientSession(read, write) as session:
                                    ...
                                    logger.info(
                                        f"Discovered {tools_discovered} tools from SSE server {server_config.name}"
                                    )
                            ...
                            if "tools" in locals() and tools:
                                logger.info(
                                    f"Registering {len(tools)} tools from SSE server {server_config.name}"
                                )
                                ...
                                for tool in tools:
                                    ...
                                    composer.composed_server._tool_manager._tools[tool_name] = (
                                        tool_obj
                                    )
                                    composer.source_mapping[tool_name] = server_config.name

                                logger.info(
                                    f"Successfully registered {len(tools)} tools from SSE server {server_config.name}"
                                )
                                print(f"    Tools: {len(tools)} registered", file=out)
                                print("    Status: ✓ Connected", file=out)
                            else:
                                print("    Status: ❌ No tools discovered", file=out)
                        except Exception as e:
                            ...
                            if "TaskGroup" in error_str and "sub-exception" in error_str:
                                ...
                                sse_tools_count = sum(
                                    1
                                    for name in composer.source_mapping
                                    if composer.source_mapping[name] == server_config.name
                                )
                                if sse_tools_count > 0:
                                    logger.warning(
                                        f"SSE server {server_config.name} connected successfully ({sse_tools_count} tools) but had cleanup issues: {e}"
                                    )
                                    print(
                                        f"    Status: ⚠️  Connected ({sse_tools_count} tools, cleanup warning)",
                                        file=out,
                                    )
                                else:
                                    logger.error(
                                        f"SSE server {server_config.name} failed during tool registration: {e}"
                                    )
                                    print("    Status: ❌ Tool registration failed", file=out)
                            else:
                                logger.error(
                                    f"Failed to connect to SSE server {server_config.name}: {e}"
                                )
        ...
        if (
            hasattr(config, "servers")
            and hasattr(config.servers, "proxied")
            and hasattr(config.servers.proxied, "http")
        ):
            from mcp import ClientSession
            ...
            if http_servers:
                ...
                for server_config in http_servers:
                    if isinstance(server_config, HttpProxiedServerConfig):
                        ...
                        print(f"    URL: {server_config.url}", file=out)
                        print(
                            f"    Protocol: {server_config.protocol.value if hasattr(server_config.protocol, 'value') else server_config.protocol}",
                            file=out,
                        )
                        ...
                        try:
                            ...
                            protocol_value = (
                                ...
                                if hasattr(server_config.protocol, "value")
                                else str(server_config.protocol)
                            )
                                ...
                                if server_config.auth_token:
                                    if server_config.auth_type.lower() == "bearer":
                                        headers["Authorization"] = (
                                            f"Bearer {server_config.auth_token}"
                                        )
                                    elif server_config.auth_type.lower() == "basic":
                                        headers["Authorization"] = (
                                            f"Basic {server_config.auth_token}"
                                        )
                                    else:
                                ...
                                async with streamablehttp_client(
                                    ...
                                    async with ClientSession(read_stream, write_stream) as session:
                                        ...
                                        for tool in tools:
                                            ...
                                            def make_streamable_http_proxy(
                                                http_config,
                                                ...
                                                async def streamable_http_tool_proxy(**kwargs):
                                                    ...
                                                    if http_config.auth_token:
                                                        if (
                                                            http_config.auth_type.lower()
                                                            == "bearer"
                                                        ):
                                                            hdrs["Authorization"] = (
                                                                f"Bearer {http_config.auth_token}"
                                                            )
                                                        elif (
                                                            http_config.auth_type.lower() == "basic"
                                                        ):
                                                            hdrs["Authorization"] = (
                                                                f"Basic {http_config.auth_token}"
                                                            )
                                                        else:
                                                            hdrs["Authorization"] = (
                                                ...
                                                streamable_http_tool_proxy.__name__ = (
                                                    tool_name.replace("-", "_")
                                                )
                                                streamable_http_tool_proxy.__doc__ = (
                                                    tool_description
                                                )
                                                return streamable_http_tool_proxy

                                            # Create the proxy function
                                            proxy_func = make_streamable_http_proxy(
                                    ...
                                    for tool in tools:
                                        ...
                                        def make_http_proxy(http_config, original_tool_name: str):
                                            ...
                                            async def http_tool_proxy(**kwargs):
                                                ...
                                                from .transport.http_stream import (
                            ...
                            logger.error(
                                f"Failed to connect to HTTP server {server_config.name}: {e}"
                            )
                            print(f"    Status: ❌ Connection failed: {e}", file=out)
                            # Print detailed traceback for debugging
                            import traceback
        ...
        if (
            hasattr(config, "servers")
            and hasattr(config.servers, "proxied")
            and hasattr(config.servers.proxied, "streamable_http")
        ):
            import subprocess
            import time
            ...
            if streamable_http_servers:
                print(
                    f"Connecting to {len(streamable_http_servers)} Streamable HTTP server(s)...",
                    file=out,
                )
                print(file=out)
                    ...
                    if isinstance(server_config, StreamableHttpProxiedServerConfig):
                        ...
                        if server_config.auto_start and server_config.command:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Failed to auto-start Streamable HTTP server {server_config.name}: {e}"
                                )
                                print(f"    Auto-start failed: {e}", file=out)
                        ...
                        try:
                            ...
                            async with streamablehttp_client(
                                ...
                                async with ClientSession(read_stream, write_stream) as session:
                                    ...
                                    tools_discovered = len(tools)
                                    logger.info(
                                        f"Discovered {tools_discovered} tools from Streamable HTTP server {server_config.name}"
                                    )
                            ...
                            if "tools" in locals() and tools:
                                logger.info(
                                    f"Registering {len(tools)} tools from Streamable HTTP server {server_config.name}"
                                )
                                ...
                                for tool in tools:
                                    ...
                                    # Create a proxy function for this streamable HTTP tool
                                    def make_streamable_http_proxy(
                                        http_config, original_tool_name: str, tool_description: str
                                    ):
                                        """Create a proxy function that calls the remote streamable HTTP server."""
                                        ...
                                        streamable_http_tool_proxy.__name__ = tool_name.replace(
                                            "-", "_"
                                        )
                                ...
                                logger.info(
                                    f"Successfully registered {len(tools)} tools from Streamable HTTP server {server_config.name}"
                                )
                                print(f"    Tools: {len(tools)} registered", file=out)
                                print("    Status: ✓ Connected", file=out)
                            else:
                            ...
                            if "TaskGroup" in error_str and "sub-exception" in error_str:
                                # This is a cleanup issue that happens after tools are registered
                                # Check if tools were actually registered by looking at composer
                                streamable_http_tools_count = sum(
                                    1
                                    for name in composer.source_mapping
                                    if composer.source_mapping[name] == server_config.name
                                )
                                if streamable_http_tools_count > 0:
                                    logger.warning(
                                        f"Streamable HTTP server {server_config.name} connected successfully ({streamable_http_tools_count} tools) but had cleanup issues: {e}"
                                    )
                                    print(
                                        f"    Tools: {streamable_http_tools_count} registered",
                                        file=out,
                                    )
                                    print("    Status: ✓ Connected (cleanup warning)", file=out)
                                else:
                                    logger.error(
                                        f"Streamable HTTP server {server_config.name} failed during tool registration: {e}"
                                    )
                                    print("    Status: ❌ Tool registration failed", file=out)
                            else:
                                logger.error(
                                    f"Failed to connect to Streamable HTTP server {server_config.name}: {e}"
                                )
                                print(f"    Status: ❌ Connection failed: {e}", file=out)
                                # Print detailed traceback for debugging
                                import traceback
        ...
        if transport_mode == "stdio":
            ...
            print("=" * 70, file=out)
            print("✓ Unified MCP server is ready!", file=out)
        ...
        if config.authentication and config.authentication.enabled:
            ...
            if oauth2_config and oauth2_config.client_id and oauth2_config.client_secret:
                from .api.routes.oauth import configure_oauth
