...
if sys.version_info >= (3, 11):
    ...
    try:
        ...
        raise ImportError(
            "tomli package is required for Python < 3.11. Install it with: pip install tomli"
        )
...
def load_config(config_path: str | Path) -> MCPComposerConfig:
    """
    ...
    try:
        ...
        raise MCPConfigurationError(
            f"Failed to parse TOML configuration: {e}", config_path=str(config_path)
        ) from e
    ...
    try:
        ...
        raise MCPConfigurationError(
            f"Invalid configuration: {e}", config_path=str(config_path)
        ) from e
...
def load_config_from_dict(config_data: dict, config_dir: str | None = None) -> MCPComposerConfig:
    """
...
def find_config_file(
    start_dir: str | Path | None = None, filename: str = "mcp_compose.toml"
) -> Path | None:
    """
...
def _substitute_env_vars_in_config(
    config: MCPComposerConfig, config_dir: str | None = None
) -> MCPComposerConfig:
    """
...
def validate_config_file(config_path: str | Path) -> tuple[bool, str | None]:
    """
