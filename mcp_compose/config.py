...
class TransportConfig(BaseModel):
    ...
    streamable_http_enabled: bool = Field(
        default=True, description="Enable Streamable HTTP transport"
    )
    streamable_http_path: str = Field(default="/mcp", description="Streamable HTTP endpoint path")
    streamable_http_cors_enabled: bool = Field(
        default=True, description="Enable CORS for Streamable HTTP"
    )
...
class JwtAuthConfig(BaseModel):
    ...
    issuer: str | None = Field(default=None, description="JWT issuer")
    audience: str | None = Field(default=None, description="JWT audience")
...
class OAuth2AuthConfig(BaseModel):
    ...
    discovery_url: str | None = Field(
        default=None, description="OAuth2 discovery URL (deprecated, use issuer_url)"
    )

    # Explicit endpoints (alternative to issuer_url auto-discovery)
    authorization_endpoint: str | None = Field(
        default=None, description="Authorization endpoint for upstream OAuth2 provider"
    )
    token_endpoint: str | None = Field(
        default=None, description="Token endpoint for upstream OAuth2 provider"
    )
    userinfo_endpoint: str | None = Field(
        default=None, description="UserInfo endpoint URL for token validation"
    )
    ...
    audience: str | None = Field(default=None, description="Expected audience claim")
    required_scopes: list[str] | None = Field(
        default=None, description="Required scopes for access"
    )
    user_id_claim: str = Field(default="sub", description="Claim to use for user ID extraction")

    # OAuth2 authorization flow options (for client-side auth)
    redirect_uri: str | None = Field(
        default=None, description="Redirect URI for OAuth2 authorization flow"
    )
    scopes: list[str] | None = Field(
        default=None, description="OAuth2 scopes to request during authorization"
    )
...
class AuthenticationConfig(BaseModel):
    ...
    api_key: ApiKeyAuthConfig | None = Field(default=None)
    basic: BasicAuthConfig | None = Field(default=None)
    jwt: JwtAuthConfig | None = Field(default=None)
    oauth2: OAuth2AuthConfig | None = Field(default=None)
    mtls: MtlsAuthConfig | None = Field(default=None)
    anaconda: AnacondaAuthConfig | None = Field(default=None)
...
class RateLimitingConfig(BaseModel):
    ...
    per_role_limits: dict[str, int] = Field(
        default_factory=dict, description="Per-role rate limits"
    )
...
class ResourceLimits(BaseModel):
    ...
    max_memory_mb: int | None = Field(default=None, description="Maximum memory in MB")
    max_cpu_percent: int | None = Field(default=None, description="Maximum CPU percentage")
...
class StdioProxiedServerConfig(BaseModel):
    ...
    health_check_tool: str | None = Field(
        default=None, description="Tool to invoke for health check"
    )
    resource_limits: ResourceLimits | None = Field(default=None)
...
class SseProxiedServerConfig(BaseModel):
    ...
    health_check_endpoint: str | None = Field(default=None, description="Health check endpoint")
    ...
    command: list[str] | None = Field(
        default=None, description="Command to start the server (if auto_start=true)"
    )
    env: dict[str, str] = Field(
        default_factory=dict, description="Environment variables for auto-started server"
    )
    working_dir: str | None = Field(
        default=None, description="Working directory for auto-started server"
    )
...
class HttpProxiedServerConfig(BaseModel):
    ...
    protocol: HttpStreamProtocol = Field(
        default=HttpStreamProtocol.LINES, description="Streaming protocol"
    )
    auth_token: str | None = Field(default=None, description="Authentication token")
    auth_type: str = Field(default="bearer", description="Authentication type")
    timeout: int = Field(default=30, description="Request timeout in seconds")
    ...
    poll_interval: int = Field(
        default=2, description="Polling interval in seconds (for poll protocol)"
    )
    health_check_enabled: bool = Field(default=False, description="Enable health checks")
    health_check_interval: int = Field(default=60, description="Health check interval in seconds")
    health_check_endpoint: str | None = Field(default=None, description="Health check endpoint")
...
class EmbeddedServersConfig(BaseModel):
    ...
    servers: list[EmbeddedServerConfig] = Field(
        default_factory=list, description="List of embedded servers"
    )
...
class ProxiedServersConfig(BaseModel):
    ...
    stdio: list[StdioProxiedServerConfig] = Field(
        default_factory=list, description="STDIO proxied servers"
    )
    sse: list[SseProxiedServerConfig] = Field(
        default_factory=list, description="SSE proxied servers"
    )
    http: list[HttpProxiedServerConfig] = Field(
        default_factory=list, description="HTTP streaming proxied servers"
    )
    streamable_http: list[StreamableHttpProxiedServerConfig] = Field(
        default_factory=list, alias="streamable-http", description="Streamable HTTP proxied servers"
    )
...
class CustomTemplateConfig(BaseModel):
    ...
    template: str = Field(
        default="{server_name}_{tool_name}", description="Template string for tool naming"
    )
...
class ToolManagerConfig(BaseModel):
    ...
    conflict_resolution: ConflictResolutionStrategy = Field(
        default=ConflictResolutionStrategy.PREFIX, description="Global conflict resolution strategy"
    )
...
class ApiConfig(BaseModel):
    ...
    cors_methods: list[str] = Field(
        default_factory=lambda: ["GET", "POST", "PUT", "DELETE"], description="CORS allowed methods"
    )
...
class UiConfig(BaseModel):
    ...
    static_dir: str | None = Field(default=None, description="Static files directory")
...
class LoggingConfig(BaseModel):
    ...
    aggregate_managed_logs: bool = Field(
        default=True, description="Aggregate logs from managed processes"
    )
...
class HealthConfig(BaseModel):
    ...
    detailed_endpoint: str = Field(
        default="/health/detailed", description="Detailed health check endpoint"
    )
    ...
    def validate_config(self) -> "MCPComposerConfig":
        ...
        for stdio_server in self.servers.proxied.stdio:
            if stdio_server.health_check_enabled:
                if (
                    stdio_server.health_check_method == HealthCheckMethod.TOOL
                    and not stdio_server.health_check_tool
                ):
                    raise ValueError(
                        f"Server {stdio_server.name}: health_check_method is 'tool' but health_check_tool not specified"
                    )
